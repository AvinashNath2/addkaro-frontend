...
function DiscountPill({ label, pct }: { label: string; pct: number | null | undefined }) {
  if (!pct) return null
  return (
    <div className="flex flex-col items-center bg-green-50 border border-green-200 rounded-xl py-3 px-2 text-center">
      <span className="text-xl font-extrabold text-green-600">{pct}%</span>
      <span className="text-[11px] text-green-500 mt-0.5">{label}</span>
    </div>
  )
}
...
function AvailabilityCalendar({ availability }: { availability: string }) {
  ...
  function dayClass(date: Date | null) {
    ...
    if (availability === 'BOOKED')   return 'bg-red-100 text-red-700 rounded font-bold text-[10px]'
    if (availability === 'AVAILABLE') return 'bg-green-100 text-green-700 rounded text-[10px]'
    ...
    <Section icon={<Calendar className="w-4 h-4" />} title="Availability — Next 6 Months" accent="green">
      <div className="flex items-center gap-3 text-xs mb-3 mt-1">
        <span className={cn('font-medium px-2.5 py-0.5 rounded-full text-xs',
          isAvail ? 'bg-green-100 text-green-700' : isBooked ? 'bg-red-100 text-red-700' : 'bg-yellow-100 text-yellow-700'
        )}>
          {isAvail ? 'Open for Booking' : isBooked ? 'Currently Booked' : 'Partially Available'}
        </span>
        <span className="flex items-center gap-1 text-gray-400"><span className="w-2.5 h-2.5 rounded-sm bg-green-200 inline-block"/>Free</span>
        <span className="flex items-center gap-1 text-gray-400"><span className="w-2.5 h-2.5 rounded-sm bg-red-200 inline-block"/>Booked</span>
      </div>
            ...
            <div key={`${year}-${month}`}>
              ...
              <div className="grid grid-cols-7 gap-px">
                {['S','M','T','W','T','F','S'].map((d, i) => (
                  <div key={i} className="text-[9px] text-gray-300 text-center pb-0.5">{d}</div>
                ))}
                {cells.map((date, i) => (
          ...
          <div className="grid grid-cols-2 gap-3 pt-1">
            {audience.dailyVehiclesRange && (
              <div className="bg-sky-50 border border-sky-100 rounded-xl p-3">
                <p className="text-[10px] uppercase tracking-wide text-sky-500 mb-1">Daily Vehicles</p>
            ...
            {audience.dailyFootfallRange && (
              <div className="bg-purple-50 border border-purple-100 rounded-xl p-3">
                <p className="text-[10px] uppercase tracking-wide text-purple-500 mb-1">Daily Footfall</p>
        ...
        <Section icon={<Zap className="w-4 h-4" />} title="Amenities & Features" accent="brand">
          {amenities.powerSupplyType && (
            <p className="text-xs text-gray-500 bg-gray-50 rounded-lg px-3 py-2 mb-3">
              Power: <span className="font-medium text-gray-700">{amenities.powerSupplyType.replace(/_/g, ' ')}</span>
            </p>
          )}
          ...
          <div className="flex items-center gap-2 mb-3 mt-1">
            {legal.permitStatus && (
              <span className={cn('text-xs font-bold px-3 py-1 rounded-full',
                legal.permitStatus === 'YES'     ? 'bg-green-100 text-green-700' :
                legal.permitStatus === 'PENDING' ? 'bg-amber-100 text-amber-700' :
...
export default function HoldingDetailPage() {
  ...
  const offerMutation = useMutation({
    ...
    onSuccess: (data) => { setSubmittedOfferId(data.offerId); reset() },
  })
    ...
    <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          {/* ── Hero photo card ─────────────────────────────────────────── */}
          <div className="relative rounded-2xl overflow-hidden shadow-md bg-gray-900">
            ...
            <div className="flex gap-3 min-w-max">
              ...
              <div className="flex items-center gap-2.5 bg-white border border-gray-100 rounded-xl px-4 py-3 shadow-sm">
                ...
                <div>
                  ...
                  <p className="text-sm font-semibold text-green-700">{formatRupees(holding.rentalCost)}</p>
                </div>
              </div>
              ...
              {holding.illumination?.isIlluminated !== undefined && (
                <div className="flex items-center gap-2.5 bg-white border border-gray-100 rounded-xl px-4 py-3 shadow-sm">
                  ...
                  <div>
                    <p className="text-[10px] text-gray-400 uppercase tracking-wide">Illuminated</p>
              ...
              {holding.pricing?.minimumBookingMonths && (
                <div className="flex items-center gap-2.5 bg-white border border-gray-100 rounded-xl px-4 py-3 shadow-sm">
                  ...
                  <div>
                    <p className="text-[10px] text-gray-400 uppercase tracking-wide">Min. Booking</p>
          ...
          <div className="sticky top-6 space-y-3">
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
              <div className="bg-gradient-to-br from-brand-600 to-brand-700 px-6 py-5 text-center">
                ...
                <p className="text-brand-200 text-sm mt-0.5">per month</p>
              ...
              <div className="p-4 space-y-3">
                ...
                {/* Non-customer */}
                {user && !isCustomer && (
                  <p className="text-center text-sm text-gray-400 py-2">Only customers can submit offers.</p>
                )}

                {/* Booked notice */}
                {isCustomer && holding.availability !== 'AVAILABLE' && (
                ...
                {isCustomer && holding.availability === 'AVAILABLE' && (
                  <>
                    ...
                    {submittedOfferId ? (
                      ...
                      <>
                        <h3 className="font-semibold text-gray-900 text-sm pt-1">Submit an Offer</h3>
                        <form
                        ...
                        >
                          ...
                          <div>
                            ...
                            />
                            {errors.desiredDuration && <p className="error-text">{errors.desiredDuration.message}</p>}
                          </div>
                          ...
                          <button type="submit" disabled={offerMutation.isPending} className="btn-primary">
            ...
            {holding.ownerType && (
              <div className="bg-white rounded-2xl border border-gray-100 shadow-sm px-4 py-3 flex items-center gap-2">
                ...
                <span className="text-xs text-gray-500">
                  Owner: <span className="font-semibold text-gray-700">{holding.ownerType}</span>
                </span>
              </div>
            )}
          </div>
